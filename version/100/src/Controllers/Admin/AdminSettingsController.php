...
class AdminSettingsController extends AdminController
{
    ...
    public function handle(): string
    {
        ...
        if (isset($this->request['registerWebhooks']) && $this->request['registerWebhooks']) {
            ...
            $adapter->getApi()->deleteAllWebhooks();
    ...
    {
        $registered = [];
        $registeredHooks = $adapter->getApi()->fetchAllWebhooks();

        foreach ($registeredHooks as $webhook) {
            /** @var Webhook $webhook */
            $registered[] = $webhook->getEvent();
        }

        $newEvents = array_diff(array_keys(PaymentEventSubscriber::getSubscribedEvents()), $registered);
