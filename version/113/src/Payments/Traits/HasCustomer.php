...
trait HasCustomer
{
    ...
    protected function createOrFetchHeidelpayCustomer(
        ...
        if ($session->has(SessionHelper::KEY_CUSTOMER_ID) && $session->get(SessionHelper::KEY_CUSTOMER_ID) != -1) {
            ...
            if (!empty($frontSession->Customer()->cFirma) && empty($customer->getCompany())) {
                $customer->setCompany(
                    html_entity_decode(utf8_encode($frontSession->Customer()->cFirma), ENT_COMPAT, 'UTF-8')
                );
            }
            ...
            if (!$isB2B) {
                $customer->setFirstname(
                    html_entity_decode(
                        utf8_decode($frontSession->Customer()->cVorname),
                        ENT_COMPAT,
                        'UTF-8'
                    )
                );
                $customer->setLastname(
                    html_entity_decode(
                        utf8_decode($frontSession->Customer()->cNachname),
                        ENT_COMPAT,
                        'UTF-8'
                    )
                );

                // Remove Company Information as we do not want that
